...
node {
    ...
    def SF_USERNAME='devopriyo.seal@wise-narwhal-q99ij0.com'                                                        //env.HUB_ORG_DH
    def SERVER_KEY_CREDENTALS_ID=env.JWT_CRED_ID_DH
    def TEST_LEVEL='RunLocalTests'
    def PACKAGE_NAME='04t7S000000oyvXQAQ'
    def PACKAGE_VERSION
    ...
    withEnv(["HOME=${env.WORKSPACE}"]) {
        ...
        withCredentials([file(credentialsId: SERVER_KEY_CREDENTALS_ID, variable: 'server_key_file')]) {
            ...
            stage('Push To Test Scratch Org') {
                rc = bat returnStatus: true, script: "\"${toolbelt}/sfdx\" force:source:push --targetusername ciorg"
                if (rc != 0) {
            ...
            stage('Run Tests In Test Scratch Org') {
                rc = bat returnStatus: true, script: "\"${toolbelt}/sfdx\" force:apex:test:run --targetusername ciorg --wait 10 --resultformat tap --codecoverage --testlevel ${TEST_LEVEL}"
                if (rc != 0) {
            ...
            stage('Delete Test Scratch Org') {
                rc = bat returnStatus: true, script: "\"${toolbelt}/sfdx\" force:org:delete --targetusername ciorg --noprompt"
                if (rc != 0) {
            ...
            stage('Create Package Install Scratch Org') {
                rc = bat returnStatus: true, script: "\"${toolbelt}/sfdx\" force:org:create --targetdevhubusername HubOrg --setdefaultusername --definitionfile config/project-scratch-def.json --setalias installorg --wait 10 --durationdays 1"
                if (rc != 0) {
            ...
            stage('Display Install Scratch Org') {
                rc = bat returnStatus: true, script: "\"${toolbelt}/sfdx\" force:org:display --targetusername installorg"
                if (rc != 0) {
            ...
            stage('Install Package In Scratch Org') {
                rc =bat returnStatus: true, script: "\"${toolbelt}/sfdx\" force:package:install --package ${PACKAGE_VERSION} --targetusername installorg --wait 10"
                if (rc != 0) {
            ...
            stage('Run Tests In Package Install Scratch Org') {
                rc = bat returnStatus: true, script: "\"${toolbelt}/sfdx\" force:apex:test:run --targetusername installorg --resultformat tap --codecoverage --testlevel ${TEST_LEVEL} --wait 10"
                if (rc != 0) {
